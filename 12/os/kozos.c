...
getcurrent(void)
{
	...
	readyqueue[current->priority].head = current->next;
	if (readyqueue[current->priority].head == NULL)
		readyqueue[current->priority].tail = NULL;
...
putcurrent(void)
{
	...
	if (readyqueue[current->priority].tail) {
		readyqueue[current->priority].tail->next = current;
	} else {
		readyqueue[current->priority].head = current;
	}
	readyqueue[current->priority].tail = current;
	current->flags |= KZ_THREAD_FLAG_READY;
...
schedule(void)
{
	...
	for (i = 0; i < PRIORITY_NUM; i++) {
		if (readyqueue[i].head)
			break;
	}
	if (i == PRIORITY_NUM)
		kz_sysdown();

	current = readyqueue[i].head;
}
